...
/* — ISR flags ————————————————————————————————————————————— */
volatile uint8_t flag_ev      = 0;   /* set by TIM1 every 10 ms */
volatile uint8_t sensor_flag = 0 ;   /* set by TIM3 every 100 ms */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	  ...
	  if(htim->Instance==TIM3) sensor_flag=1;  /*100 ms EVtick    */
}
...
int main(void)
{
	  ...
	  EV_Init(&ev);           // ev initializing
	  HCSR04_Init();          // ultra sonic sensor iniializing
	  ADAS_Init(&adas);        // initializing l r f  ->400cm
	  ...
	  HAL_TIM_Base_Start(&htim2);         /*count every 1us     */
	  ...
	  while (1)
	  {
          ...
          if(sensor_flag ==1)    //100ms tick
          {
        	  ...
        	  /* validating motor temp,soc,colllision  based on this update the fault flag */
