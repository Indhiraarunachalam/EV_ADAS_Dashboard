...
float ultrasonic_read(GPIO_TypeDef* TrigPort, uint16_t TrigPin, GPIO_TypeDef* EchoPort, uint16_t EchoPin)
{
    ...
    while(!(HAL_GPIO_ReadPin(EchoPort, EchoPin))) {
        if ((HAL_GetTick() - timeout) > 20) return 0;
    }
    ...
    if (stop > start) {
        ...
        time_local = (65535 - start) + stop;
    }

    distance_local = ((float)time_local * 0.0343f) / 2.0f;
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  distance[0] = (uint32_t)ultrasonic_read(GPIOA, GPIO_PIN_0, GPIOA, GPIO_PIN_1);
	  HAL_Delay(30);

	  distance[1] = (uint32_t)ultrasonic_read(GPIOA, GPIO_PIN_2, GPIOA, GPIO_PIN_3);
	  HAL_Delay(30);

	  distance[2] = (uint32_t)ultrasonic_read(GPIOA, GPIO_PIN_4, GPIOA, GPIO_PIN_5);

	  sprintf(uart_buf, "S1: %ld cm  |  S2: %ld cm  |  S3: %ld cm\r\n", distance[0], distance[1], distance[2]);
